...
public class SJFPreemptive implements Scheduler {
    ...
    public void run(){
        ...
        int n = list.size(), idx = 0, done = 0, time = 0;
        PriorityQueue<Proc> pq = new PriorityQueue<>(Comparator.comparingInt(a->a.remaining));
        Proc running = null;
        while(done < n){
        ...
        try{ StatsWriter.writeOutputs("SJF_Preemptive",gantt,procs,cs,csTime);}catch(IOException e){}
    }
}
